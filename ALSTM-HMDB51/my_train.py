...
model_dir = '../tfs_saver/'
dataset = 'ucf11_4'
save_name = dataset+'_train_lri_'+str(lr_rate)+'_batch_size'+str(batch_size)+'_hidden_size_'+str(hidden_size)+'_num_layers_'+str(num_layers)
...
def inference(images,batch_size,True_or_False=False):
	...
	with tf.variable_scope("RNN"):
		...
		for time_step in range(num_steps):
			...
			Xt = images[:, time_step, :, :, :]
			lt_squar_expend=tf.expand_dims(lt_squar, 3)
			t_Xt = tf.mul(Xt,lt_squar_expend)
	...
	return outputs,trainable_variables,lt_out,outputs_for_loss


def loss_v2(outputs_for_loss, labels,lt_out,trainable_variables):
	cross_entropy = 0.
	for i in range(num_steps):
		cross_entropy+=tf.nn.sparse_softmax_cross_entropy_with_logits(outputs_for_loss[:,i,:],labels[:,i])

	final = 0.
	for i in trainable_variables:
		final+=tf.nn.l2_loss(i)
	final_lost = globle_gamma*final+cross_entropy/30.0
...
def train(batch_size):
	with tf.Graph().as_default():
		...
		global_step = tf.get_variable('global_step', [], tf.int32, initializer=tf.constant_initializer(0),
									  trainable=False)
		...
		lt_out = tf.transpose(lt_out, [1, 0, 2, 3])
		...
		lost = loss_v2(outputs_for_loss, labels, lt_out, trainable_variables)
		# lost = (64,)
